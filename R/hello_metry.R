#' Wrapper function going row-by-row to estimate size (mm) and mass (mg)
#'
...
#' has some mass measurements but not everywhere, please name this column "mass_mg", and include
#' another column "mass_kind" that says if the mass is "dry" or "wet". This is important as this
...
#' @param mass_kind Should data used in inference be "dry" for just dry mass, or "both" (default) for both dry and wet mass.
...
#' of size estimation (NA if not done), total mass for given row (in mg), 
#' as well as path taken through the function. The nomenclature of this path column is as follows.
#' ## Special cases
#'- "null_mass": abundance was 0
#'- "_size_estimaton_failed": there were not enough close relatives or species with matching traits to compute size
#'- "_mass_estimation_failed": there were no allometric equation at to estimate mass
#'- "_raw_dry/wet": if there was a raw (direct) measurement for that particular species-size combination, and whether that measurement is dry or wet mass
...
#'  mass estimation:taxonomic level of inference_number of equations_dry/wet.
#'  For example, -AE:bwg_name_wet (allometric equation from wet mass at the species level), -AE:subclass_dry (allometric equation from dry mass at the subclass level)
#' # If a species went through both size estimation and mass estimations, the path will be composite, e.g. WA:genus_5-AE:bwg_name_wet


#' @export
hello_metry <- function(data_table, mass_kind = "dry", database = TRUE){
  #browser()
  ...
  # If mass column is present make a fork here
  if("mass_mg" %notin% colnames(data_table))
    {data_return <- 
      data_table %>% 
  ...
  for(i in 1:nrow(data_return)){
    ...
    row <- data_return[i,]
    mass <- row$mass
    ## If we already have a mass value go to the next iteration
    if(!is.na(mass)) next
   ...
   ### Make small frame to get taxonomy
     c(taxo <- 
        ...
        unique(),
     ### If not in database, give special mass value
      if(nrow(taxo) == 0)
        c(taxo <- 
            row %>% 
    ...
    if(is.numeric(size_mm)){
    #### As long as mass is NA
    #### Vector of three elements, so using first one
      ##### Go through my list of group
      for(level in level_list){
        c(est <- 
            get_biomass(specname, level, size_mm, abundance, stage0, path, taxo, measurement_table, mass_kind),
        path <- 
          est[4],
        mass <- 
          as.numeric(est[1:3]),
        ...
        if(is.na(mass[1]) & level == "class")
          c(mass <-
              firetruck(size_mm, abundance, taxo),
              ifelse(is.na(mass[1]),
                     c(mass <- rep("cannot_estimate", 3),
                       path <- paste0(path, "_cannot_estimate")),
                     c(mass <- c(signif(as.numeric(mass[1]), 
                                           digits = 4),
    ...
    ### Add mass and path value to data frame to return
    ...
    data_return[i,(ncol(data_return)-1):(ncol(data_return)-3)] <- 
      matrix(as.character(rev(mass)), nrow = 1)
